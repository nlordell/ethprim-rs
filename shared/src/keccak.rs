//! A `const` Keccak-256 implementation.
...
/// implementation here is split between the [`V256::absorb`] and
/// [`V256::squeeze`] and specialized for Keccak-256.
...
impl V256 {
    ...
    /// [`Sponge::pad`]-ing or [`Sponge::squeeze`]-ing data from the sponge.
...
mod tests {
    ...
    fn keccak256() {
        for (data, digest) in [
            ...
            assert_eq!(hex(v256(data.as_bytes())), digest);
        }
    }
}
